...
async function verifyAssetUsageSwidTag(res, assetUsage) {
    ...
    if (!swidTag) {
        denials.addDenial(assetUsage, "swidTagNotFound",
            `swid-tag not found for swTagId(${assetUsage.swTagId})`);
    }
    if (!licenseProfile) {
        denials.addDenial(assetUsage, "licenseProfileNotFound",
            `license-profile not found for swTagId(${assetUsage.swTagId})
            with licenseProfileId(${(swidTag || '').licenseProfileId || ''})`);
    }
}

/**
 * check the RTU for a single assetUsageId
 * @param  {} res
 * @param  {} assetUsage either assetUsage or includedAssetUsage
 */
async function checkRtu(res, assetUsage) {
    ...
    denials.addDenial(assetUsage, "agreementNotFound",
