...
/**
 * generate sql command to either find the RTU or select all preentitled swid-tags
 * @param  {boolean} getSwidTagForUsage whether to retrieve all preentitled (false)
...
function genSqlToEntitle(getSwidTagForUsage, keys, actionField, userField, reqUsageCountField) {
    ...
    if (getSwidTagForUsage) {
        ...
        sqlOptions.entitlement = `"preentitlement"`;
...
module.exports = {
    ...
    /**
     * get preentitled swid-tags from database per userId and action
     * @param  {} res
     */
    async getPreentitledSwidTags(res) {
        if (!res.locals.params.action || !res.locals.params.userId) {
            utils.logInfo(res, `skipped getPreentitledSwidTags(${res.locals.paramKeys})`);
            return;
        }
        utils.logInfo(res, `in getPreentitledSwidTags(${res.locals.paramKeys})`);
        ...
        result.rows.forEach(est => {if (est.preentitlement == null) {delete est.preentitlement;}});
        res.locals.response.preentitledSwidTags = result.rows;
        utils.logInfo(res, `out getPreentitledSwidTags(${res.locals.paramKeys}): ${result.rows.length} rows`);
    }
 };
