...
before(function() {
    ...
    console.log(`before all testing ${process.env.NODE_VERSION} ${(new Date()).toISOString()}`);
    mockRequire('pg', './mock-pg');
});

after(function() {
    console.log(`done all testing ${(new Date()).toISOString()} ${utils.milliSecsToString(utils.now())}`);
    mockRequire.stop('pg');
});

describe('lum-server', function () {
    before(async function() {
        console.log(`before testing lum-server - require('../lum-server')`);
        ...
        for await (const waitStep of asyncCountGenerator(5)) {
            if (lumServer.healthcheck.databaseInfo) {
                console.log(`lumServer ready[${waitStep}]: ${lumServer.config.serverName}`);
                lumServer.logger.info(`lumServer ready[${waitStep}]: ${lumServer.config.serverName}`);
                break;
            }
            console.log(`lumServer waiting[${waitStep}] for init: ${lumServer.config.serverName}`);
        ...
        console.log(`lumServer inited: ${lumServer.config.serverName}`, lumServer.healthcheck);
    ...
    console.log(`lumServer testing: ${JSON.stringify(expectationFiles)}`);
