...
- new API endpoint ``GET /api/v1/swid-tags/preentitled`` that returns the collection of
  swidTag records preentitled for the userId to perform the requested action at this moment.
  (`ACUMOS-3735 <https://jira.acumos.org/browse/ACUMOS-3735>`_)

  * This API ``GET /api/v1/swid-tags/preentitled`` does not increment the usage counters
...
- major refactoring of the logic behind ``PUT /api/v1/asset-usage`` and ``PUT /api/v1/asset-usage-event``
  that is related to the new API ``GET /api/v1/swid-tags/preentitled`` to avoid code duplication.
