...
#. verifies usage **count** on the permission for the specific **action**
    ...
    .. list-table:: usage constraints on permission
        ...
        * - **action** "acumos:deploy"
          ...
          -
        * - **action** in ["install", "acumos:run"]
          - ``{"action": ["install", "acumos:run"], "constraint": [{"leftOperand": "count", "operator": "lteq", "rightOperand": {"@value": "45", "@type": "xsd:integer"}}]}``
          - each action has a separate limit of 45
        * - **action** "acumos:train"
          - ``{"action": "acumos:train", "constraint": [{"leftOperand": "count", "operator": "lteq", "rightOperand": {"@value": "15", "@type": "xsd:integer"}}]}``
          - model training
        * - **action** "acumos:compose.include"
          - ``{"action": "acumos:compose.include", "constraint": [{"leftOperand": "count", "operator": "lteq", "rightOperand": {"@value": "55", "@type": "xsd:integer"}}]}``
          - include the model in composition
